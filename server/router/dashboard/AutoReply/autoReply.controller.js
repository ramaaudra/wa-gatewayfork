...
export default class AutoReplyController extends AutoReply {
  ...
  async createReply(req, res) {
    try {
      ...
      if (!userId) {
        return res
          .status(401)
          .send({ status: 401, message: "Unauthorized. Please log in." });
      }
      ...
      let check = await this.checkExistAutoReply(
        session_number,
        keyword,
        userId
      );
      ...
      if (check) {
        ...
        if (mediaLibraryId) {
          ...
          const mediaItem = await MediaModel.findOne({
            where: { id: mediaLibraryId, user_id: userId },
          });
          if (mediaItem) {
            ...
            console.warn(
              `Media item with ID ${mediaLibraryId} not found for user ${userId} or does not exist.`
            );
            return res
              .status(404)
              .send({
                status: 404,
                message: `Media item not found in your library.`,
              });
          }
        } else if (req.file) {
          const file = req.file;
  ...
  async editReply(req, res) {
    try {
      ...
      const existingReply = await this.checkMessageUser(
        session,
        keyword,
        userId
      );
      if (!existingReply) {
        return res
          ...
          .send({
            status: 404,
            message: "Auto-reply not found or you do not have permission.",
          });
      }
      ...
      const deleteOldMediaIfNeeded = (currentReply) => {
        if (
          currentReply &&
          currentReply.media_url &&
          currentReply.media_url.startsWith("/media/autoreply/")
        ) {
          // Only delete if it's an autoreply media
  ...
  async deleteReply(req, res) {
    try {
      ...
      if (
        existingReply.media_url &&
        existingReply.media_url.startsWith("/media/autoreply/")
      ) {
        const publicPath = path.join(__dirname, "../../../../public");
    ...
    try {
      ...
      if (allReplies && allReplies.length > 0) {
        ...
        for (const reply of allReplies) {
          if (
            reply.media_url &&
            reply.media_url.startsWith("/media/autoreply/")
          ) {
            ...
            if (fs.existsSync(mediaPath)) {
              try {
                fs.unlinkSync(mediaPath);
                console.log(
                  "Deleted media on deleteAllReply for user:",
                  userId,
                  mediaPath
                );
              } catch (delError) {
                console.error(
                  "Error deleting media on deleteAllReply for user:",
                  userId,
                  delError
                );
              }
