    ...
    try {
      ...
      if (array && array.length > 0) {
        ...
        for (const value of array) {
          // Use for...of for async operations in loop
          // Original logic: value.status = "STOPPED"; // This doesn't save
          await value.update({ status: "STOPPED" });
        }
      }
    } catch (error) {
      console.error("Error in startProgram:", error);
    }
  }
  ...
  async deleteInactiveSessions(userId = null) {
    ...
    try {
      ...
      if (Array.isArray(sessions) && sessions.length > 0) {
        console.log(
          `Found ${sessions.length} inactive sessions to delete for user_id: ${
            userId || "all users"
          }.`
        );
        ...
        for (const session of sessions) {
          console.log(
            `Deleting inactive session: ${session.session_name} (User: ${session.user_id})`
          );
        ...
        return {
          ...
          message: `Successfully deleted ${
            sessions.length
          } inactive sessions for user_id: ${userId || "all users"}.`,
          count: sessions.length,
        };
      } else {
        return {
          success: true,
          message: `No inactive sessions found for user_id: ${
            userId || "all users"
          }.`,
          count: 0,
        };
      ...
      console.error(
        `Error deleting inactive sessions for user_id ${
          userId || "all users"
        }:`,
        error
      );
