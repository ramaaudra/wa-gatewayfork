...
import initUserModel from "../database/models/user.model.js";
import initSessionModel from "../database/models/session.model.js";
import initMediaModel from "../database/models/media.model.js";
import initHistoryModel from "../database/models/history.model.js";
import initAutoReplyModel from "../database/models/autoReply.model.js";
import initButtonResponseModel from "../database/models/buttonRespon.model.js";
import initListResponseModel from "../database/models/listRespon.model.js";
...
const History = initHistoryModel(sequelize, User);
const Session = initSessionModel(sequelize, User, History);
const MediaModel = initMediaModel(sequelize, User);
const AutoReplyModel = initAutoReplyModel(sequelize, User);
const ButtonResponseModel = initButtonResponseModel(sequelize, User);
const ListResponseModel = initListResponseModel(sequelize, User);
...
async function connectDatabase() {
  connection.query(`use \`${DB_NAME}\`;`, (err) => {
    if (err) {
      console.log(
        ...
        modules.color(`Create new Database Success!`, "#82E0AA")
      );
  ...
  await sequelize.sync(syncOptions).then(() => {
    console.log(
      modules.color("[APP]", "#EB6112"),
      modules.color(moment().format("DD/MM/YY HH:mm:ss"), "#F8C471"),
      modules.color(`Re-Sync All Tables (with alter:true)`, "#82E0AA")
    );
  });
